...
export default class StackTransitioner extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const routeAnimationType =
      action === PUSH
        ? children && children.props.animationType
        : previousChildren && previousChildren.props.animationType;
  ...
  render() {
      ...
      previousChildren &&
        routes.push(
          <Animated.View key={previousChildren.key} style={this.getRouteStyle(0)}>
            {!this.isFixed('footer') && PreviousFooter && (
              <PreviousFooter animatedValue={this.animatedValue} />
            )}
            {previousChildren}
            {!this.isFixed('header') && PreviousHeader && (
              <PreviousHeader animatedValue={this.animatedValue} />
            )}
          </Animated.View>
        );
      children &&
    ...
    return (
      ...
      >
        ...
        <View style={styles.transitionContainer}>{routes.map(route => route)}</View>
